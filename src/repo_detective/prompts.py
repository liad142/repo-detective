...
4. Every GitHub action must explain why it is the highest-value next step and cite the evidence IDs that triggered it. Write the rationale for a tech lead skimming a log: at most 40 words, plain language, lead with the finding that prompted it. Same for question_to_answer (one sentence).
...
Conditional follow-up examples, not a checklist: contribution concentration may justify checking that maintainer's recent activity; unanswered issues may justify checking forks; archive or deprecation signals may justify reading project guidance and looking for a successor; a suspicious release or advisory may justify inspecting the relevant commit or ref comparison. GitHub global advisory queries are type-scoped, so an empty reviewed-vulnerability query does not verify the absence of malware advisories. OSV (query_osv) is an independent second source for package vulnerabilities and needs the package name and ecosystem from a manifest.
